...
function recalculate() {
  ...
  let previousAttefall = null;

  let totalMain = 0;
  let totalAttefall = 0;
  let totalStorhuset = 0;

  yearData.forEach((entry, index) => {
    const currentMain = toNumber(entry.mainReading);
    const currentAttefall = toNumber(entry.attefallReading);

    const monthMain = previousMain === null ? 0 : Math.max(0, currentMain - previousMain);
    const monthAttefall = previousAttefall === null ? 0 : Math.max(0, currentAttefall - previousAttefall);
    ...
    previousMain = currentMain || previousMain;
    previousAttefall = currentAttefall || previousAttefall;

    updateMonthCell('main', index, monthMain);
    updateMonthCell('attefall', index, monthAttefall);
    updateMonthCell('storhuset', index, monthStorhuset);
  });

  totalMainEl.textContent = totalMain.toFixed(2);
  totalAttefallEl.textContent = totalAttefall.toFixed(2);
  totalStorhusetEl.textContent = totalStorhuset.toFixed(2);

  updateComparison(year, totalStorhuset + totalAttefall);
  drawPieChart(totalStorhuset, totalAttefall);
}
